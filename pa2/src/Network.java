...
public class Network extends Thread {
    ...
    private static Semaphore Client_Input;
    private static Semaphore Server_Input;

    private static Semaphore Client_Output;
    private static Semaphore Server_Output;
    ...
    Network( )
    {
        ...
        Server_Input = new Semaphore(0);
        Client_Input = new Semaphore(maxNbPackets);

        Client_Output = new Semaphore(0);
        Server_Output = new Semaphore(maxNbPackets);
    ...
    public void run()
    {
        ...
        while (true)
        {
            if(getClientConnectionStatus().equals("disconnected") && getServerConnectionStatus().equals("disconnected")) {
                ...
                System.out.println("\n Terminating network thread - Client " + getClientConnectionStatus() + ", Server " + getServerConnectionStatus() + ". Running Time " + (networkEndTime - networkStartTime) + " ms."   );
